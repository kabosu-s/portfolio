...
export default async function SnippetsIndex() {
  ...
  return (
    <main className="max-w-4xl mx-auto py-12">
      <header className="mb-16 p-8 bg-primary border-4 border-text shadow-large">
        ...
        {snippets.length === 0 ? (
          ...
          snippets.map((post) => (
            <Link key={post.slug} href={`/snippets/${post.slug}`} className="group block">
              <article className="p-8 border-4 border-text bg-surface shadow-large group-hover:translate-x-[2px] group-hover:translate-y-[2px] group-hover:shadow-hover transition-all">
                ...
                <h2 className="text-3xl font-black mt-2 text-text group-hover:text-secondary transition-colors">
                  {post.title}
                </h2>
                <p className="mt-4 text-text font-medium leading-relaxed text-lg">
                  {post.description}
                </p>
